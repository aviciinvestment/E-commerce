...
class AdminService {
  ...
  async adjustOrderStatus(orderId, newStatus) {
    ...
    if (!validStatuses.includes(newStatus))
      throw new Error("Invalid logistics status assignment configuration.");
  ...
  async reviewVendorApplication(vendorId, decisionStatus) {
    ...
    if (!validDecisions.includes(decisionStatus)) {
      throw new Error(
        "Invalid vendor application status assignment designation.",
      );
    }
    ...
    if (!updatedVendor)
      throw new Error("Target vendor application profile missing.");
  ...
  async approveVendorPayoutTicket(withdrawalId, transactionReference) {
    ...
    if (ticket.status !== "Pending") {
      throw new Error(
        `This payout ticket has already been processed as ${ticket.status}.`,
      );
    }

    if (!transactionReference) {
      throw new Error(
        "A clear banking ledger transaction reference code is required for authorization.",
      );
    }
  ...
  // ==========================================
  // FETCH PLATFORM DATA FOR DASHBOARD TABLES
  // ==========================================
  ...
  async getPlatformOrders() {
    // Optionally use .populate('user', 'name email') if your schema references the User model
    return await Order.find().sort({ createdAt: -1 });
  }

  async getPlatformVendors() {
