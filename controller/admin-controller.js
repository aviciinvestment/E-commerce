...
const UpdateUserStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `User status configuration updated safely to active: ${isActive}`,
    });
  } catch (error) {
...
const UpdateOrderStatusByAdmin = async (req, res) => {
  try {
    ...
    const updatedOrder = await adminService.adjustOrderStatus(
      orderId,
      newStatus,
    );
    return res.status(200).json({
      success: true,
      message: `Logistics pipeline updated to ${newStatus}`,
      data: updatedOrder,
    });
  } catch (error) {
    return res.status(400).json({ success: false, error: error.message });
...
const ModerateReviewForceDelete = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Review removed cleanly by admin moderation.",
    });
  } catch (error) {
    return res.status(400).json({ success: false, error: error.message });
  }
...
const AdminApprovePayout = async (req, res) => {
  try {
    ...
    const completedPayout = await adminService.approveVendorPayoutTicket(
      withdrawalId,
