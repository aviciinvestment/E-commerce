...
const RegisterVendorStore = async (req, res) => {
  try {
    ...
    // ⚡ 2. UPDATE THE USER TO LINK THE NEW VENDOR ID
    ...
    return res.status(201).json({
      success: true,
      message: "Vendor application filed. Waiting for admin approval.",
      data: newVendor,
    });
  } catch (error) {
...
const RequestPayout = async (req, res) => {
  try {
    ...
    const payoutReceipt = await vendorService.initiateWithdrawalRequest(
      vendorId,
      Number(amount),
    );
